...
class PayslipPdfController extends Controller
{
    public function view(int $entry)
    {
        ...
        $earnings = $entry->items
            ->filter(fn($item) => $item->item_type === 'Earning')
            ->sortByDesc('finalized_amount');

        $deductions = $entry->items
            ->filter(fn($item) => $item->item_type === 'Deduction')
            ->sortByDesc('finalized_amount');
    ...
    //     $earnings = $entry->items->component
    //         ->filter(fn($i) => $i->type === 'Earning')
    //         ->sortByDesc('finalized_amount');

    //     $deductions = $entry->items->component
    //         ->filter(fn($i) => $i->type === 'Deduction')
    //         ->sortByDesc('finalized_amount');
